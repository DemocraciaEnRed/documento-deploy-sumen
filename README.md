...
Los permisos para el almacenamiento y las carpetas del proveedor deben permanecer en "775", por razones obvias de seguridad.

Sin embargo, tanto su computadora como su servidor Apache deben poder escribir en estas carpetas. 

Ej: cuando ejecuta comandos como `php artisan`, su computadora necesita escribir en el archivo de registros en el almacenamiento.

Todo lo que necesita hacer es ceder la propiedad de las carpetas a Apache: 

```
sudo chown -R www-data:www-data /var/www/sumen-app/vendor
sudo chown -R www-data:www-data /var/www/sumen-app/storage
```
